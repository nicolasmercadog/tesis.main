...
void AsyncMqttClient::_insert(AsyncMqttClientInternals::OutPacket* packet) {
  ...
  SEMAPHORE_TAKE();
  //log_d("new insert #%u", packet->packetType());
...
void AsyncMqttClient::_addFront(AsyncMqttClientInternals::OutPacket* packet) {
  ...
  SEMAPHORE_TAKE();
  //log_d("new front #%u", packet->packetType());
  if (_head == nullptr) {
    _tail = packet;
  } else {
    packet->next = _head;
...
void AsyncMqttClient::_addBack(AsyncMqttClientInternals::OutPacket* packet) {
  SEMAPHORE_TAKE();
  //log_d("new back #%u", packet->packetType());
  if (!_tail) {
    _head = packet;
  } else {
    _tail->next = packet;
...
void AsyncMqttClient::_handleQueue() {
  ...
  while (_head && _client.space() > 10) {  // safe but arbitrary value, send at least 10 bytes
    ...
    if (_head->size() > _sent) {
      ...
      _lastPingRequestTime = 0;
      #if ASYNC_TCP_SSL_ENABLED
      //log_d("snd #%u: (tls: %u) %u/%u", _head->packetType(), realSent, _sent, _head->size());
      #else
      //log_d("snd #%u: %u/%u", _head->packetType(), _sent, _head->size());
      #endif
    ...
    if (_head->size() == _sent) {
      if (_head->released()) {
        //log_d("p #%d rel", _head->packetType());
  ...
  SEMAPHORE_GIVE();
  if (disconnect) {
    //log_d("snd DISCONN, disconnecting");
    _client.close();
  }
}
  ...
  while (packet) {
    ...
    if (keepSessionData) {
      if (packet->qos() > 0 && packet->size() <= _sent) {  // check for qos includes check for PUB-packet type
        ...
        //log_d("keep #%u", packet->packetType());
        SEMAPHORE_GIVE();
        _addBack(packet);
        SEMAPHORE_TAKE();
        packet = next;
